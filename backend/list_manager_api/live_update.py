...
async def all_updates(request: Request):
    return EventSourceResponse(sse_message_publisher(request=request, list_id=None), ping=60)
...
async def specific_list_updates(list_id: int, request: Request):
    ...
    return EventSourceResponse(sse_message_publisher(request=request, list_id=list_id), ping=60)
